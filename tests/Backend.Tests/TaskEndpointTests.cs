...
public class TaskEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task TaskEndpoints_FullCrudFlow_SucceedsAccordingToContract()
    {
        ...
        // 4. Obtener tarea por ID (200 OK)
        var getByIdResponse = await client.GetAsync($"/api/tasks/{createdTask.Id}");
        Assert.Equal(HttpStatusCode.OK, getByIdResponse.StatusCode);

        var retrievedTask = await getByIdResponse.Content.ReadFromJsonAsync<TaskItem>();
        Assert.NotNull(retrievedTask);
        Assert.Equal(createdTask.Id, retrievedTask.Id);
        Assert.Equal("Integración continua", retrievedTask.Title);

        // 5. Toggle estado de tarea (200 OK)
        ...
        // 6. Eliminar tarea (204 No Content)
        var deleteResponse = await client.DeleteAsync($"/api/tasks/{createdTask.Id}");
        Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);

        // 7. Intentar obtener tarea eliminada (404 Not Found)
        var notFoundGet = await client.GetAsync($"/api/tasks/{createdTask.Id}");
        Assert.Equal(HttpStatusCode.NotFound, notFoundGet.StatusCode);

        // 8. Intentar togglear tarea inexistente / eliminada (404 Not Found)
        ...
        // 9. Intentar eliminar tarea inexistente (404 Not Found)
        var notFoundDelete = await client.DeleteAsync($"/api/tasks/{createdTask.Id}");
